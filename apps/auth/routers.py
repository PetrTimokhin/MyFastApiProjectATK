...
@auth_router.post("/login", response_model=Token, summary="Авторизация пользователя")
...
@auth_router.post(
    "/token/refresh", response_model=Token, summary="Обновляем Access Token"
)
def refresh_token_endpoint(refresh_token_str: str = Depends(get_current_refresh_token)):
